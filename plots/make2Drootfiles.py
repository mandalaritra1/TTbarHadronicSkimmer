...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-data",
        action="store_true",
        help="Do not load/write the Data ROOT file (e.g. signal-only builds that "
             "reuse an existing Data ROOT input).",
    )
    parser.add_argument(
        "--skip-ttbar",
        action="store_true",
        help="Do not load/write the TTbar ROOT file (signal-only builds).",
    )
    parser.add_argument(
        "--ttbar-pattern",
        default=None,
...
def _zprime_signal_specs(
    ...
    """Return ``(output_label, mass, width)`` triples for requested ZPrime signals."""
    ...
    for mass_value in masses:
        ...
        for width_value in widths:
            ...
            specs.append((output_label, mass, width))
    return specs


def _zprime_signal_point_specs(points: list[str], year: str) -> list[tuple[str, str, str]]:
    specs = []
    for point in points:
...
def _check_unique_signal_specs(specs: list[tuple[str, str, str]]) -> list[tuple[str, str, str]]:
    seen: dict[str, tuple[str, str]] = {}
    for output_label, mass, width in specs:
        if output_label in seen:
            raise ValueError(
                f"Duplicate signal output label {output_label!r} from "
                f"{seen[output_label]} and (mass={mass}, width={width})"
            )
        seen[output_label] = (mass, width)
    return specs


def _resolve_signal_outputs(coffea_dir: Path, year: str, mass: str, width: str, util_module) -> list[dict]:
    """One ZPrime mass point as a per-mass output, from either a legacy per-mass
    file (ZPrime<mass>_<width>_<year>*.coffea) or a grouped width-file
    (ZPrime<width>_<year>*.coffea) selected on the dataset axis."""
    label = f"signalZPrime{mass}"
    per_mass = sorted(coffea_dir.glob(f"ZPrime{mass}_{width}_{year}*.coffea"))
    if per_mass:
        print(f"Loading {label} (per-mass): {per_mass[0]}")
        return [util_module.load(per_mass[0])]
    ds_label = f"ZPrime{mass}_{width}"
    for path in sorted(coffea_dir.glob(f"ZPrime{width}_{year}*.coffea")):
        out = util_module.load(path)
        if sg.is_grouped(out) and ds_label in sg.dataset_labels(out):
            print(f"Loading {label} (grouped {path.name} -> {ds_label})")
            return [sg.select_mass(out, ds_label)]
    raise FileNotFoundError(
        f"No coffea for ZPrime mass {mass} width {width}: looked for per-mass "
        f"ZPrime{mass}_{width}_{year}*.coffea and grouped ZPrime{width}_{year}*.coffea "
        f"containing dataset {ds_label!r} in {coffea_dir}"
    )
...
def _load_outputs(paths: list[Path], label: str, util_module) -> list[dict]:
    ...
    for path in paths:
        print(f"  {path}")
        out = util_module.load(path)
        if sg.is_grouped(out):
            # grouped width-file: expand to one per-mass view per dataset label
            for m in sg.dataset_labels(out):
                print(f"    -> {m}")
                outputs.append(sg.select_mass(out, m))
        else:
            outputs.append(out)
    return outputs
...
def main() -> None:
    ...
    if ttbar_pattern and not args.skip_ttbar:
    ...
    if args.signal_points or args.signal_masses:
        ...
        for signal_label, mass, width in signal_specs:
            signal_outputs = _resolve_signal_outputs(coffea_dir, args.year, mass, width, util)
