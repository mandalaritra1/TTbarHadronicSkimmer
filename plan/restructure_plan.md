...
- 2026-05-01 (Codex): Fixed signal output naming in `python/run_analysis.py`. `ZPrimeLocal` no longer leaves a dangling subsection underscore before option suffixes, so `ZPrime4000_Local_2024__noSyst_test.coffea` becomes `ZPrime4000_Local_2024_noSyst_test.coffea`.
- 2026-05-01 (Codex): Added notebook `Print CLI only` mode. `python.interactive_config.build_cli_command(args)` prints the equivalent `ttbaranalysis.py` command from selected widgets, and the notebook run cell skips `run_analysis(args)` when the checkbox is enabled.
- 2026-05-01 (Codex): Added explicit chunksize override across CLI, notebook UI, notebook command printing, and static HTML command builder. `--chunksize 0` / UI value `0` preserves the existing auto defaults (`100` in test mode, otherwise `200000` futures and `100000` Dask).
